...
<body>
	...
	<nav aria-label="Page navigation"
		...
		<ul class="pagination">
			...
			<li class="page-item"><a class="page-link text-black"
				href="list_one.jsp?pagenow=<%=i%>&pagenowc=<%=pagenowc%>"><%=i%></a></li>
			<%
			}
			%>
		</ul>
	...
	// 전체 글 수 조회
	int totalPostsc = daocom.selectListNum(); // getTotalPosts()는 전체 글 수를 반환하는 메소드
	...
	<div class="container" style="padding-top: 50px;">
		<table class="table table-hover">
			...
			<tbody class="table-group-divider">
				...
				<tr>
					<th scope="row"><a
						href="../_board/list_one.jsp?writer=<%=commen.getWriter()%>"><%=commen.getWriter()%></a></th>
	...
	<nav aria-label="Page navigation"
		...
		<ul class="pagination">
			...
			%>
			<li class="page-item"><a class="page-link text-black"
				href="list_one.jsp?pagenowc=<%=i%>&pagenow=<%=pagenow%>"><%=i%></a></li>
			<%
			}
			%>
		</ul>
